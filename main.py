...
sorted_calories = {k: v for k, v in sorted(products_dict.items(), key=lambda item: item[1]['calorie'], reverse=True)}
...
for item in get_menus_json()['data']['menus'][0]['menu_categories'][2]['products']:
    ...
    if product_name in sorted_calories:
        sorted_calories[product_name]['price'] = item['product_variations'][0]['price']

print(tabulate([[key, value] for key, value in sorted_calories.items()], headers=['Termék', 'Kcal']))
